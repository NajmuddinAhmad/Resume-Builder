...
async function aiCheckATS() {
  ...
  try {
    ...
    showAIResults(`
      ...
      ${((analysis.strengths || data.strengths)||[]).map(s=>`<div class="suggestion-item">• ${esc(s)}</div>`).join('')}
      <div style="margin:8px 0"><strong>🎯 Improvements:</strong></div>
      ${((analysis.improvements || data.improvements)||[]).map(s=>`<div class="suggestion-item">• ${esc(s)}</div>`).join('')}
    `, 'ATS Analysis');
...
async function aiRewriteField(field, content, section) {
  ...
  try {
    showToast('Rewriting with AI...', 'info', 2000);
    const data = await API.ai.rewrite({ content, context: section, tone: 'professional' });
